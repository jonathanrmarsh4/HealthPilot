...
function SidebarContentWrapper({ 
  ...
  return (
    <>
      <div 
        className={`flex ${isNativePlatform() ? 'h-screen overflow-hidden' : 'min-h-dvh'} w-full max-w-full bg-background overflow-x-hidden`}
        style={isMobile ? {
          width: '100%',
          maxWidth: '100%',
          paddingLeft: 0,
      ...
      >
        ...
        <div className="flex flex-col flex-1 min-w-0">
          ...
          <header className="md:hidden fixed top-0 left-0 right-0 z-50 backdrop-blur-xl bg-background/80 border-b border-border">
            <div 
              className="flex items-center justify-end gap-2 py-2 px-4"
      ...
      {isMobile && (
        <MobileNav
          ...
          LinkComponent={Link}
          onChatClick={location !== "/chat" && !isChatOpen && !isVoiceChatOpen ? () => {
...
function AppLayout() {
  ...
  // Force wrapper to full viewport width on mobile and zero out all padding/gutters
  const style = isMobile ? {
    "--sidebar-width": "0px",
    "--sidebar-width-icon": "0px",
    "--sidebar-mobile-gutter": "0px",
    width: "100vw",
    minWidth: "100vw",
    maxWidth: "100vw",
    flexBasis: "100vw",
