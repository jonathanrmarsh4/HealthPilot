...
- **SmartFuel™ Precision Nutrition Guidance System:** Evidence-based nutrition recommendation system that replaces legacy Spoonacular-based meal recommendations. Analyzes biomarkers (cholesterol, blood pressure, glucose), health goals, and dietary preferences to generate personalized "what to avoid" and "what to include" guidance. Built on YAML-based clinical rule packs (18 evidence-based rules for conditions like hypertension, elevated LDL, prediabetes) and a JSON food ontology (100+ categorized foods). Uses a reasoning engine (signal normalizer, risk profiler, target setter) and NLG template system to convert structured data into friendly, actionable advice. Legacy meal/recipe features parked using FEATURE_SHOW_RECIPE_FEATURES flag (default: false) for potential future reactivation.
