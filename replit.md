...
- **Baseline Mode & Feature Flag System (Oct 2025 - ✅ COMPLETED):** Comprehensive feature flag infrastructure (`shared/config/flags.ts`) enabling progressive AI/ML feature rollout. **BASELINE_MODE_ENABLED** (default: `true`) acts as master override forcing all AI features OFF, providing stable deterministic catalog functionality. System includes 8 typed flags: AI_MEAL_FILTERS_ENABLED, AI_MEAL_RANKING_ENABLED, MEAL_GOAL_FILTER_ENABLED, MEAL_PREFERENCE_WEIGHTING_ENABLED, BIOMARKER_FILTER_ENABLED, AI_WORKOUT_SELECTION_ENABLED, EXERCISE_MEDIA_AUTOMAP_ENABLED. All flags default to `false`, work on both frontend (via `import.meta.env.VITE_*`) and backend (via `process.env`), and use override logic where baseline mode forces AI OFF regardless of individual flag settings. Implementation includes: (1) New GET /api/meals endpoint for deterministic catalog browsing with strict pagination (24 items/page), mealType/tag filters, and popularityScore DESC sorting; (2) MealCatalogBrowser component with category tabs (breakfast/lunch/dinner/snack), visible pagination, responsive grid layout; (3) AI guards on 4 endpoints (meal recommendations, workout generation, AI chat, daily insights) returning 403 in baseline mode; (4) ExerciseDetailsModal respecting EXERCISE_MEDIA_AUTOMAP_ENABLED for ID-only GIF lookups vs fuzzy matching; (5) Zod validation schemas for API contracts (MealSchema, ExerciseSchema); (6) Telemetry logging for baseline events; (7) Complete documentation in BASELINE_MODE.md covering flag behavior, rollout strategy, and troubleshooting. Benefits: Ship stable core features immediately, debug without AI complexity, progressively enable features, maintain fallback mode for AI service issues.
