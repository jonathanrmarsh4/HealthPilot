...
- **Premium Theme System v1.1 - Refined Liquid Glass Design (Nov 2025):** Admin-controlled visual theme featuring desaturated coral (#FFB4AF) → pink (#F0BFDC) → teal (#80E5E5) gradient with Apple-inspired Liquid Glass aesthetic. **Light Mode:** Clean white base (#FFFFFF) with 15% opacity gradient overlay for subtle elegance. **Dark Mode:** Sophisticated dark base (#0F1419) with 20% opacity gradient overlay for distinct visual separation. Glass components use 85-90% opacity for excellent content readability. Complete design system includes: (1) CSS variables for refined brand colors, glass effects, shadows, and SF Pro typography, (2) Mode-specific base colors with subtle gradient overlays, (3) Liquid Glass component (.hp-glass) with frosted blur and dual-border system (::before for outer hairline, ::after for inner sheen), (4) Premium buttons (.hp-btn-primary, .hp-btn-ghost) with gradient backgrounds and hover lift effects, (5) Glass cards, tiles, and chips with 85-90% opacity for content visibility, (6) Fluid responsive typography (.h1, .h2) using clamp() sizing, (7) Distinct light/dark mode palettes. Theme persists across all pages using premium-theme class on html/body elements. Controlled via Admin Panel CMS Theme tab with real-time toggle. Architecture: landing_page_content.premium_theme_enabled database field, GET/PUT API endpoints, PremiumThemeProvider component applying premium-theme class, explicit utility classes (.hp-*) scoped to .premium-theme namespace.
