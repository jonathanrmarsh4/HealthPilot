...
- **AI Symptom Correlation Engine v2.0 - Holistic Assessment:** Intelligent analysis system that combines ALL active symptoms into a SINGLE AI query with comprehensive biomarkers (sleep, HRV, BP, activity, medications) to identify root causes and patterns. Uses Occam's Razor principle to find the fewest causes explaining the most symptoms, implementing 6 evidence-based correlation rules with safety-first language. Symptom diagnosis/triage appears exclusively in the Daily tab for medical assessment, not in AI Coach recommendations.
