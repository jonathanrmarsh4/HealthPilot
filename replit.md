...
- **Personalized Recommendations & Automation:** AI generates personalized meal plans with feedback, macro recommendations, and exercise recommendations with smart auto-scheduling. Automated goal updates based on biomarker data. **AI-Driven Meal Curation (Oct 2025):** Enhanced meal generation creates 30-50 curated options per meal type from the meal_library database table. Each meal includes AI reasoning explaining health benefits based on biomarkers, goals, and nutrition profile. Fixed dislike filter to properly exclude all feedback types. Smart filtering by dietary preferences, allergens, macros, and disliked meals with intelligent ranking. **Meal Library Management:** Admin interface at `/admin/meal-library` for one-time import of meals from Spoonacular API into the database. Target: 500 diverse meals across 10 cuisines and 3 meal types.
...
- **Landing Page CMS (Oct 2025 - In Progress):** Custom-built content management system for the landing page at `/admin/landing-page`. Allows admin users to manage landing page content without code changes. **Completed:** Database schema with 5 tables (landing_page_content, landing_page_features, landing_page_testimonials, landing_page_pricing_plans, landing_page_social_links), storage methods and admin-protected API routes, admin interface with tabbed sections (Hero, Features, Testimonials, Pricing, Social, SEO), dynamic rendering for Hero and "How It Works" sections, initial content seeding. **Current State:** Hero section and "How It Works" section are fully dynamic and editable via CMS. Other sections (testimonials, pricing, social links, features) have backend infrastructure but frontend still uses hardcoded content. **Next Steps:** Extend dynamic rendering to remaining sections (testimonials, pricing, social, features), add media upload functionality for images, implement loading/error states, add Zod validation for admin mutations.
