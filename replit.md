...
- **HealthKit Automatic Sync v2.0 (Nov 2025):** Simple, reliable automatic sync system using app lifecycle hooks. Triggers existing manual sync endpoint on app launch and when app returns to foreground. Includes smart 5-minute cooldown to prevent excessive API calls and battery drain. Implemented via Capacitor App plugin in MobileBootstrap.ts. iOS-only feature. Maintains full backward compatibility with manual sync button.
